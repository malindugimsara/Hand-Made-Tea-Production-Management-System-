...
rawMaterialCostRoutes.post('/', verifyToken, authorizeRoles('Admin', 'Officer'), createRawMaterialCost);

// Route: GET /api/raw-material-cost
// Description: Get all records (Admin, Officer, සහ Viewer ට)
rawMaterialCostRoutes.get('/', verifyToken, authorizeRoles('Admin', 'Officer', 'Viewer'), getAllRawMaterialCosts);

// Route: PUT /api/raw-material-cost/:id
// Description: Update a specific record (Admin සහ Officer ට පමණක්)
rawMaterialCostRoutes.put('/:id', verifyToken, authorizeRoles('Admin', 'Officer'), updateRawMaterialCost);

// Route: DELETE /api/raw-material-cost/:id
// Description: Delete a specific record (Admin සහ Officer ට පමණක්)
rawMaterialCostRoutes.delete('/:id', verifyToken, authorizeRoles('Admin', 'Officer'), deleteRawMaterialCost);
