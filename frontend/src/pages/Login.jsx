...
import api from "../api/axiosConfig"; // Path eka oyalage folder structure ekata anuwa wenas wenna puluwan
...
const TeaLeaf = ({ left, top, delay, size, color }) => (
  ...
  >
    <path
      d="M12 2C6 2 2 8 2 14c0 4 2 7 6 8 1-4 2-8 4-10-2 4-3 8-2 12 2 0 4-1 5-3 1-2 1-5 1-8 0 3 1 6 3 8 4-2 5-6 5-9C24 6 18 2 12 2z"
      fill="currentColor"
    />
  </motion.svg>
);
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <rect
            width="100%"
            height="100%"
            fill={`url(#${mode === "handmade" ? "hm-grid" : "pk-grid"})`}
          />
        </motion.svg>
      </AnimatePresence>
    </div>
...
function WipeOverlay({ isWiping, direction }) {
  const wipe = direction
    ? THEMES[direction].wipeGradient
    : THEMES.handmade.wipeGradient;
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!hasAccess) {
        toast.error(
          `Access Denied: A ${data.role} cannot log into the ${activeTab === "handmade" ? "Handmade Tea" : "Packing Section"} system.`,
        );
        setIsLoading(false);
        return;
      }
      ...
      setIsSuccess(true);
      setTimeout(
        () => navigate(activeTab === "handmade" ? "/dashboard" : "/packing"),
        1500,
      );
    } catch (error) {
      // Axios error handling hariyama lesi. Server eken ena error eka mehema ganna puluwan.
      const errorMessage =
        ...
        <div className="text-center flex flex-col items-center">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === "handmade" ? (
                <>
                  <Leaf size={16} className="animate-pulse" /> Handmade Tea
                  System
                </>
              ) : (
                <>
                  <Package size={16} className="animate-bounce" /> Packing
                  Section System
                </>
              )}
            </motion.div>
          ...
          <div className="flex gap-3 mt-3">
            {["handmade", "packing"].map((sys) => (
              <button
                ...
                style={{
                  backgroundColor:
                    activeTab === sys ? t.accent : "rgba(255,255,255,0.5)",
          ...
          {!isSuccess ? (
            ...
            >
              ...
              <motion.div
                ...
                style={{
                  background: `radial-gradient(ellipse at 50% 0%,${t.shimmer} 0%,transparent 60%)`,
                }}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ duration: 0.8 }}
              ...
              <div className="flex bg-white/60 p-1.5 rounded-2xl mb-8 border border-white shadow-inner relative overflow-hidden">
                ...
                >
                  <motion.div
                    animate={
                      activeTab === "packing" ? { scale: [1, 1.25, 1] } : {}
                    }
                    transition={{ duration: 0.45 }}
                  >
                    <Package size={15} />
              ...
              <form className="space-y-5" onSubmit={handleLogin}>
                ...
                <motion.button
                  whileHover={{
                    scale: 1.02,
                    boxShadow: `0 14px 40px -8px ${t.accent}44`,
                  }}
                  whileTap={{ scale: 0.98 }}
                  type="submit"
                  disabled={isLoading}
            ...
            >
              ...
              >
                Accessing {activeTab === "handmade" ? "Production" : "Packing"}{" "}
                System...
              </motion.p>
            </motion.div>
          )}
