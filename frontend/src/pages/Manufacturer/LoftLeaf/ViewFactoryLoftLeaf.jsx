...
import { Leaf, RefreshCw, AlertCircle, FileDown, Calendar, Factory, FileSpreadsheet, X, Save, Clock, Languages, Image, FileText } from "lucide-react";
...
export default function ViewLoftLeafCount() {
 ...
 // 💡 --- DYNAMIC TRANSLATIONS FOR UI & PDF (EN / SI) ---
  ...
  // 💡 --- DIRECT SHARE ON WHATSAPP (IMAGE / PDF) ---
  const shareOnWhatsApp = async (format) => {
      ...
      try {
          ...
          if (navigator.canShare && navigator.canShare({ files: [file] })) {
              ...
              // Desktop Fallback: Download file & open Web WhatsApp
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full xl:w-auto">
          ...
          <div className="flex gap-3 w-full sm:w-auto h-10 sm:h-auto">
            <button
              ...
              className="p-2.5 px-4 flex-1 sm:flex-none justify-center bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors shadow-sm disabled:opacity-50 flex items-center gap-2"
              title="Download PDF"
            >
              <FileDown size={18} /> 
            </button>
            ...
            <div className="relative flex-1 sm:flex-none" ref={waMenuRef}>
              ...
              {isWaMenuOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-zinc-800 rounded-xl shadow-lg border border-gray-200 dark:border-zinc-700 overflow-hidden z-50 animate-in slide-in-from-top-2">
                  ...
                  <button onClick={() => shareOnWhatsApp('pdf')} className="w-full text-left px-4 py-3 text-sm font-bold text-gray-700 dark:text-gray-200 hover:bg-green-50 dark:hover:bg-zinc-700 transition-colors flex items-center gap-3 border-t border-gray-400 dark:border-zinc-700">
            ...
            <table className="w-full text-xs sm:text-sm text-center border-collapse min-w-[1300px]">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-zinc-700 bg-white dark:bg-zinc-950">
                {rankedRecords.length === 0 ? (
                  <tr>
                    <td colSpan={14} className="p-8 sm:p-12 text-center text-gray-500 dark:text-gray-400 font-medium">
                      <AlertCircle size={36} className="mx-auto mb-3 opacity-30 text-gray-400" />
                      {t.noData || "No data available for the selected date."}
                    </td>
                  </tr>
                ) : (
