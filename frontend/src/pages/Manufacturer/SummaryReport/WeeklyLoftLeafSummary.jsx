...
export default function WeeklyLoftLeafSummary() {
  ...
  const weekDates = useMemo(() => {
    ...
    const nextweekDates = useMemo(() => {
        ...
        return weekDates.map(date => {
            ...
            d.setDate(d.getDate() + 6); // Go forward exactly 7 days
  ...
  // 💡 --- RANKING TABLE DATA (Current & Prev) ---
  const rankTableData = useMemo(() => {
      return processedTable.filter(r => r.rank !== "-" && r.name !== "E").sort((a, b) => a.rank - b.rank);
  }, [processedTable]);

  const prevRankTableData = useMemo(() => {
      return prevProcessedTable.filter(r => r.rank !== "-" && r.name !== "E").sort((a, b) => a.rank - b.rank);
  }, [prevProcessedTable]);


  // 💡 --- (GRAND AVERAGE) ---
  const grandAverages = useMemo(() => {
        ...
        {loading ? (
            ...
            <div className="p-2">
                ...
                <div className="mt-8 flex flex-col md:flex-row gap-50 items-start justify-center w-full">
                            ...
                            <tbody className="bg-white dark:bg-zinc-950">
                                {prevRankTableData.map((r, i) => (
                                    <tr key={i} className="hover:bg-gray-50 dark:hover:bg-zinc-900">
                                        ...
                                        <td className="border border-gray-300 dark:border-zinc-700 p-2 font-bold pl-3 text-gray-500">{r.name}</td>
                    ...
                    <div className="flex flex-col items-center">
                        <h3 className="sm:text-xs lg:text-sm font-bold mb-3 flex items-center gap-2 text-gray-800 dark:text-gray-200 w-full justify-center">
                            <Trophy size={18} className="text-yellow-500" /> {t.rankTableTitle} <span className="font-semibold italic">({weekDates[0]} - {nextweekDates[0]})</span>
                        </h3>
                        <table className="border-collapse border border-gray-300 dark:border-zinc-700 sm:text-xs lg:text-sm text-center">
                            <thead className="bg-gray-100 dark:bg-zinc-800 text-gray-800 dark:text-gray-200">
                                <tr>
                            ...
                            <tbody className="bg-white dark:bg-zinc-950">
                                {rankTableData.map((r, i) => (
                                    <tr key={i} className="hover:bg-gray-50 dark:hover:bg-zinc-900">
                                        ...
                                        <td className="border border-gray-300 dark:border-zinc-700 p-2 font-bold pl-3">{r.name}</td>
