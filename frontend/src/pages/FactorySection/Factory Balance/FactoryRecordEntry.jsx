...
import { Calendar, Save, Trash2, Edit, Factory, Leaf, Package, RefreshCcw, PlusCircle, ArrowLeft, ListChecks, Sparkles } from 'lucide-react';
...
export default function FactoryManagement() {
    ...
    const handleAddToList = (e) => {
        ...
        // Check if date already in Pending Queue
        ...
        // Reset form but increment date if possible, else keep today
        setFormData({
            ...formData,
            greenLeafToday: '',
            dispatch: '',
    ...
    const inputStyles = "w-full p-3.5 bg-white border border-gray-200 rounded-xl font-bold text-gray-700 focus:ring-4 focus:outline-none transition-all";

    return (
        <div className="min-h-screen p-4 sm:p-6 md:p-8 font-sans transition-colors duration-300 relative" style={{ backgroundColor: THEME.pageBg }}>
            <div className="max-w-[1600px] mx-auto relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                    <div className="flex items-center gap-4">
                        <div className="w-16 h-16 rounded-2xl flex items-center justify-center shadow-sm border" style={{ backgroundColor: THEME.badgeBg, borderColor: THEME.badgeBorder, color: THEME.textPrimary }}>
                            <Factory size={32} />
                        </div>
                        <div>
                            <h2 className="text-2xl sm:text-3xl font-black" style={{ color: THEME.textPrimary }}>Factory Data Entry</h2>
                            <p className="font-semibold mt-1 uppercase tracking-wider text-sm" style={{ color: THEME.textSecondary }}>Daily Production & Dispatch Log</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        <form onSubmit={handleAddToList} className="bg-white p-5 sm:p-8 rounded-3xl shadow-sm border border-gray-100">
                            
                            {/* DATE SECTION */}
                            <div className="mb-6 pb-6 border-b border-gray-100">
                                <div className="w-full md:w-1/2">
                                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Record Date</label>
                                    <input 
                                        type="date" 
                                        name="date" 
                                        value={formData.date} 
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                ...
                                <div className="bg-gray-50/50 border border-gray-100 rounded-2xl p-5 shadow-sm">
                                    <h3 className="text-lg font-bold mb-4 flex items-center gap-2" style={{ color: THEME.textPrimary }}>
                                        <div className="p-1.5 rounded-lg" style={{ backgroundColor: THEME.badgeBg }}><Leaf size={18}/></div>
                                        Production
                                    </h3>
                                    <div className="space-y-4">
                                        <div>
                                            <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Green Leaf Today (kg)</label>
                                            <input 
                                        ...
                                        <div>
                                            <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Est. Made Tea (21.5%)</label>
                                            <div className="w-full p-3.5 border rounded-xl flex items-center h-[54px] font-black" style={{ backgroundColor: THEME.badgeBg, borderColor: THEME.badgeBorder, color: THEME.textPrimary }}>
                                                {calculatedMadeTea > 0 ? calculatedMadeTea.toFixed(3) : '0.000'} kg
                                            </div>
                                            <p className="text-[10px] mt-1.5 font-bold flex items-center gap-1" style={{ color: THEME.textSecondary }}>
                                ...
                                <div className="bg-gray-50/50 border border-gray-100 rounded-2xl p-5 shadow-sm">
                                    <h3 className="text-lg font-bold mb-4 flex items-center gap-2" style={{ color: THEME.textSecondary }}>
                                        <div className="p-1.5 bg-gray-100 rounded-lg"><Package size={18}/></div>
                                        Dispatch & Sales
                                    </h3>
                                    <div className="space-y-4">
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Dispatch</label>
                                                <input 
                                            ...
                                            <div>
                                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Local Sales</label>
                                                <input 
                                        ...
                                        <div>
                                            <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Total Out</label>
                            ...
                            <div className="mb-8 bg-gray-50/50 border border-gray-100 rounded-2xl p-5 shadow-sm md:w-1/2">
                                <h3 className="text-lg font-bold text-gray-700 mb-4 flex items-center gap-2">
                                    <div className="p-1.5 bg-gray-200 rounded-lg"><RefreshCcw size={18}/></div>
                                    Returns
                                </h3>
                                <div>
                                    <label className="block text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Return Amount (kg)</label>
                                    <input 
                        ...
                        <div className="bg-white rounded-3xl shadow-sm border border-gray-200 flex-1 flex flex-col overflow-hidden sticky top-6">
                            ...
                            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-gray-50/50">
                                {pendingRecords.length === 0 ? (
                                    <div className="h-full flex flex-col items-center justify-center text-gray-400 py-10">
                                        ...
                                        <p className="text-sm font-bold text-gray-500">Queue is empty</p>
                                        <p className="text-xs mt-1 text-gray-400">Fill the form and add records here.</p>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        {pendingRecords.map((item, index) => (
                                            <div key={index} className="bg-white p-4 border border-gray-200 rounded-2xl shadow-sm relative group hover:border-teal-300 transition-colors">
                                                ...
                                                <div className="flex flex-col gap-3 pr-8">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-2 text-xs font-medium text-gray-600">
                                                        <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-100">
                                                            ...
                                                            <span className="font-black" style={{ color: THEME.textPrimary }}>{item.greenLeafToday} kg</span>
                                                        </div>
                                                        <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-100">
                                                            <span className="block text-[10px] uppercase text-gray-400 font-bold mb-1">Made Tea</span>
                                                            <span className="font-black" style={{ color: THEME.textSecondary }}>{item.calculatedMadeTea.toFixed(2)} kg</span>
                                                        </div>
                                                    </div>
                                                    <div className="bg-gray-50 p-2 rounded-lg border border-gray-100 flex justify-between px-3 text-xs font-bold text-gray-600">
                                                        <span>Total Out:</span>
                                                        <span className="text-orange-600">{item.calculatedTotalOut.toFixed(2)} kg</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                            ...
                            <div className="p-5 border-t border-gray-200 bg-white space-y-3">
                                <button 
                                    ...
                                    style={!(isSavingAll || pendingRecords.length === 0) ? { background: THEME.btnGradient } : {}}
                                >
