...
import { Settings2, ArrowDownCircle, ArrowUpCircle, Package, Leaf, Search, ShieldAlert, Calendar } from 'lucide-react';
...
export default function StockAdjustment() {
    ...
    const fetchStockData = async () => {
        ...
        try {
            ...
            const [teaRes, rawRes] = await Promise.all([
                ...
                // 👇 මෙතන API URL එක ඔයාගේ Main Raw Material Stock එක දෙන URL එකට මාරු කරන්න.
    ...
    const filteredItems = useMemo(() => {
        if (activeTab === 'tea') {
            ...
            return rawMaterials.filter(item => {
                ...
                const name = item.materialName || item.itemName || item.name || 'Unknown Material';
    ...
    const currentStockInfo = useMemo(() => {
        ...
        if (activeTab === 'tea') {
            const tea = teaStocks.find(t => (t.productName || t.name) === selectedItem);
            if (!tea) return null;
            return {
            ...
            const raw = rawMaterials.find(r => (r.materialName || r.itemName || r.name) === selectedItem);
            if (!raw) return null;
            return {
    ...
    const handleSubmit = async (e) => {
        ...
        if (adjustmentType === 'remove' && currentStockInfo && Number(amount) > currentStockInfo.qty) {
        ...
        try {
            ...
            toast.error("Error adjusting stock. Please check server.", { id: toastId });
        } finally {
            ...
            <div className="max-w-4xl mx-auto relative z-10">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div className="flex border-b border-gray-200 bg-gray-50">
                        <button 
                            className={`flex-1 py-4 text-sm font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-colors ${activeTab === 'tea' ? 'bg-white text-[#0d5e4d] border-b-2 border-[#0d5e4d]' : 'text-gray-500 hover:bg-gray-100'}`}
                            onClick={() => { setActiveTab('tea'); setSelectedItem(''); setSearchQuery(''); }}
                        >
                            <Leaf size={18} /> Tea Stock
                        </button>
                        <button 
                            className={`flex-1 py-4 text-sm font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-colors ${activeTab === 'raw' ? 'bg-white text-[#0d5e4d] border-b-2 border-[#0d5e4d]' : 'text-gray-500 hover:bg-gray-100'}`}
            ...
            <div className="px-4 py-3 text-gray-400 text-sm italic">No items found</div>
                                ...
                                {currentStockInfo && (
                                    <div className="bg-[#f0fdfa] border border-[#99f6e4] rounded-2xl p-4 flex items-center justify-between transition-all">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-[11px] font-bold text-teal-700 uppercase tracking-wider">Current Overall Stock</p>
                                ...
                                {adjustmentType === 'remove' && (
                                    <div className="flex items-start gap-3 p-3 bg-orange-50 border border-orange-200 rounded-xl text-orange-800 text-sm font-semibold">
                                        ...
                                        <p>You are about to reduce the overall inventory. Please ensure the amount is correct to avoid negative stock.</p>
                                    </div>
                                )}
                                ...
                                >
                                    {isSubmitting ? "Processing..." : `Confirm ${adjustmentType === 'add' ? 'Addition' : 'Issue'}`}
                                </button>
