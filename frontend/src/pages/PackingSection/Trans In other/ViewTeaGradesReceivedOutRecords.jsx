...
import { Calendar, RefreshCw, FileText, Weight, Tag, FilterX, Building, ArrowDownToLine } from "lucide-react";
...
const getTeaColor = (grade) => {
    ...
    if (p.includes('green')) return 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200 border-green-200 dark:border-green-800/50'; 
...
export default function ViewTeaReceivedOtherRecords() {
    ...
    // Type Filter එක අයින් කර ඇත, මොකද ඔක්කොම IN records විතරයි
    ...
    const fetchRecords = async () => {
        ...
        try {
            ...
            const processedData = recordsData.map(rec => {
                ...
                // Search string එකට partyName එකත් එකතු කර ඇත
    ...
    const filteredRecords = records.reduce((acc, record) => {
        ...
        if (gradeFilter) {
            ...
            // 1. Transaction No හෝ Party Name එකට සමානද කියලා බලනවා (Partial Match)
            const isTransMatch = record.transactionNo?.toLowerCase().includes(searchVal);
            const isPartyMatch = record.partyName?.toLowerCase().includes(searchVal);
            
            // 2. ඒ දෙකටම ගැලපුනේ නැත්නම්, හරියටම Grade එකට සමාන (Exact Match) items විතරක් ගන්නවා
            if (!isTransMatch && !isPartyMatch) {
                matchedItems = matchedItems.filter(
                    item => item.grade?.toLowerCase() === searchVal
                );
            }
        }
        ...
        if (matchedItems.length > 0) {
            // Filter වුන items වල එකතුව (Daily Total) අලුතෙන් හදනවා
            const filteredTotalQty = matchedItems.reduce((sum, item) => sum + (Number(item.qtyKg) || 0), 0);

            acc.push({
                ...record,
                itemsArray: matchedItems,
                totalQtyKg: filteredTotalQty // අලුත් එකතුව replace කරනවා (එතකොට Grand Total එකත් හරියටම හැදෙනවා)
            });
        }
    ...
    filteredRecords.forEach(record => {
        grandTotal += (Number(record.totalQtyKg) || 0);

        record.itemsArray.forEach(item => {
            if (!gradeSummaryMap[item.grade]) {
                gradeSummaryMap[item.grade] = 0;
            }
    ...
    const handleEditClick = (record) => {
        navigate('/packing/edit-received-out-record', { state: { recordData: record } }); // Adjust route appropriately
    };
    ...
    const uniqueCode = `TR/OTHER/${new Date().toLocaleString('default', { month: 'short' }).toUpperCase()}.${new Date().getFullYear()}`;
        ...
        <div className="p-4 sm:p-8 max-w-[1600px] mx-auto font-sans relative min-h-screen transition-colors duration-300">
            <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto mt-2 sm:mt-0">
                    <div className="w-full sm:w-auto">
                        <PDFDownloader 
                            title="Tea Received (Other Parties) Records"
                            subtitle={`Filters -> Month: ${filterMonth || 'All'} | Grade/Party: ${gradeFilter || 'All'}`}
                            headers={["Date", "Ref/Inv No", "Party", "Grade", "Qty (KG)", "Daily Total (KG)"]}
                            data={getPdfData()}
                            uniqueCode={uniqueCode}
                            fileName={`Received_Other_${new Date().toISOString().split('T')[0]}.pdf`}
            ...
            <div className="mb-6 bg-white dark:bg-zinc-900 p-5 rounded-xl border border-gray-200 dark:border-zinc-800 shadow-sm transition-colors duration-300">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <div className="flex flex-col gap-1.5 relative" ref={dropdownRef}>
                        <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Search Grade/Party/No</label>
                        <input 
                            type="text" 
                            ...
                            <table className="w-full text-sm text-left border-collapse whitespace-nowrap min-w-full">
                                <thead>
                                    <tr className="bg-gray-50 dark:bg-zinc-950/50 text-gray-500 dark:text-gray-400 uppercase text-xs tracking-wider border-b border-gray-200 dark:border-zinc-500">
                                        ...
                                        <th className="px-4 py-3 font-semibold border-r border-gray-200 dark:border-zinc-500 align-bottom"><FileText size={14} className="inline mr-1"/> Received Info</th>
                                        <th className="px-4 py-3 font-bold text-green-600 dark:text-green-500 border-r border-gray-200 dark:border-zinc-600 bg-green-50 dark:bg-green-950/30 align-bottom min-w-[160px]"><Tag size={14} className="inline mr-1"/> Grade</th>
                                        <th className="px-4 py-3 font-bold text-green-700 dark:text-green-500 border-r border-gray-200 dark:border-zinc-600 bg-green-50 dark:bg-green-950/30 text-center"><Weight size={14} className="inline mr-1"/> Qty (Kg)</th>
                                        <th className="px-4 py-3 font-bold text-gray-700 dark:text-gray-300 border-r border-gray-200 dark:border-zinc-600 text-center bg-gray-100 dark:bg-zinc-800">Total (Kg)</th>
                                    ...
                                    {filteredRecords.length > 0 ? (
                                        filteredRecords.map((record) => (
                                            <tr key={record._id} className="hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors group">
                                                ...
                                                <td className="px-4 py-4 border-r border-gray-200 dark:border-zinc-700 align-top bg-white dark:bg-zinc-900 group-hover:bg-gray-100 dark:group-hover:bg-zinc-800">
                                                    <div className="flex flex-col gap-1.5">
                                                        <span className="font-semibold text-[#0d9488] dark:text-teal-400 bg-teal-50 dark:bg-teal-900/30 px-2 py-1 rounded w-fit">{record.transactionNo}</span>
                                                        {record.partyName && (
                                                            <div className="text-xs text-gray-600 dark:text-gray-300 flex items-center gap-1 font-medium mt-1">
                                                                <Building size={12} className="text-teal-600 dark:text-teal-400"/> {record.partyName}
                                                            </div>
                                                        )}
                                                    </div>
                                                </td>
                ...
                <div className="xl:col-span-1 w-full">
                    <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-600 overflow-hidden sticky top-8">
                        <div className="bg-gray-100 dark:bg-zinc-800 px-4 py-3 border-b border-gray-200 dark:border-zinc-600">
                            ...
                            <p className="text-[10px] text-gray-500 mt-1 uppercase">Based on current filters</p>
                        </div>
                            ...
                            <table className="w-full text-sm border border-gray-300 dark:border-zinc-700 border-collapse min-w-full">
                                <thead>
                                    <tr className="bg-gray-200 dark:bg-zinc-800 border-b border-gray-300 dark:border-zinc-500">
                                        ...
                                        <th className="px-3 py-2 text-right font-bold text-green-700 dark:text-green-400">Total (Kg)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {summaryArray.length > 0 ? (
                                        summaryArray.map(([gradeName, qty], idx) => (
                                            <tr key={idx} className="border-b border-gray-300 dark:border-zinc-500">
                                                ...
                                                <td className="px-3 py-2 text-right font-medium text-green-700 dark:text-green-400 bg-green-50/30 dark:bg-green-900/10">
                                                    {qty > 0 ? (qty % 1 !== 0 ? qty.toFixed(2) : qty) : '-'}
                                                </td>
                                            </tr>
                                        ))
                                    ) : (
                                ...
                                <tfoot>
                                    <tr className="bg-gray-200 dark:bg-zinc-800 font-bold text-gray-900 dark:text-gray-100 border-t-2 border-gray-400 dark:border-zinc-500">
                                        ...
                                        <td className="px-3 py-2 text-right text-green-700 dark:text-green-400">{grandTotal % 1 !== 0 ? grandTotal.toFixed(2) : grandTotal}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
