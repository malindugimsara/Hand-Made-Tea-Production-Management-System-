...
import { PlusCircle, Trash2, ListChecks, Save, Package, ShoppingCart, Calendar, Weight, Tag, X, FileText, Building } from "lucide-react"; 
...
export default function TeaGradesReceivedOutEntry() {
    const BACKEND_URL = import.meta.env.VITE_BACKEND_URL;
    const [showSpinner, setShowSpinner] = useState(false);
    const [pendingRecords, setPendingRecords] = useState([]);
    const navigate = useNavigate();

    // Transaction Type ඉවත් කර ඇත, Party Name එකතු කර ඇත
    ...
    const handleAddToList = (e) => {
        ...
        // නව වාර්තාව නිර්මාණය කිරීම (No transactionType)
        const newRecord = { 
            date: formData.date,
                ...
                <div className="lg:col-span-3">
                    <form onSubmit={handleAddToList} className="bg-white dark:bg-zinc-900 p-6 md:p-8 rounded-2xl shadow-lg border border-teal-100 dark:border-zinc-800 transition-colors duration-300">
                        ...
                        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {/* Party Name Field - Now spans full width or fits nicely */}
                                            ...
                                            <div className="flex flex-col gap-2 pr-8">
                                                <div className="flex flex-wrap items-center justify-between gap-2">
                                                    ...
                                                    <span className="text-xs font-bold text-[#0d9488] bg-teal-50 dark:bg-teal-900/30 px-2 py-1 rounded">
                                                        REC/{record.transactionNo}
                                                    </span>
                                                </div>
                                                
                                                {record.partyName && (
                                                    <div className="text-xs text-gray-500 dark:text-gray-400 font-medium bg-white dark:bg-zinc-900 p-1.5 rounded border border-gray-100 dark:border-zinc-800 w-fit">
                                                        <Building size={12} className="inline mr-1"/> {record.partyName}
                                                    </div>
                                                )}
                                                ...
                                                <div className="bg-white dark:bg-zinc-900 p-2.5 rounded border border-gray-100 dark:border-zinc-700/50 text-xs mt-1">
                                                    ...
                                                    <div className="flex justify-between items-center font-bold">
                                                        <span className="text-gray-500 uppercase text-[10px]">Record Total:</span>
