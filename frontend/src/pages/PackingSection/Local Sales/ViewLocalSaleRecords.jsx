        ...
        pdfSortedRecords.forEach(record => {
            ...
            record.itemsArray.forEach((item, index) => {
                ...
                if (item.rawMaterialName) {
                    ...
                    const flavorQty = parseFloat(Number(item.rawMaterialQtyKg).toFixed(3)).toString();
                    rmQtys.push(item.rawMaterialQtyKg ? `${flavorQty} kg` : "-");
                }
                if (item.packingMaterials && item.packingMaterials.length > 0) {
                    item.packingMaterials.forEach(pm => {
                        if (pm.name) {
                            rmNames.push(`${pm.name} (Packing)`);
                            const pmQty = parseFloat(Number(pm.qty).toFixed(3)).toString();
                            rmQtys.push(pm.qty ? pmQty : "-");
                        }
                    });
                }
