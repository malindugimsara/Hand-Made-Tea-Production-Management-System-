...
export default function ViewTransOutRecords() {
    ...
    const filteredRecords = records.filter(record => {
        // Fallback to createdAt if dateIssued isn't explicitly set in your schema
        const rDate = new Date(record.dateIssued || record.createdAt).toISOString().split('T')[0];
    ...
    // --- AGGREGATE ISSUED QUANTITY INSTEAD OF RECEIVED ---
    const grandTotalIssuedQty = filteredRecords.reduce((sum, record) => {
        const recTotal = record.items.reduce((itemSum, item) => itemSum + (Number(item.issuedQtyKg) || 0), 0);
        return sum + recTotal;
    }, 0);
    ...
    filteredRecords.forEach(record => {
        record.items.forEach(item => {
            ...
            productSummaryMap[item.product] += Number(item.issuedQtyKg) || 0; // Summarize Issued
        });
    });
    ...
    const getPdfData = () => {
        const pdfSortedRecords = [...filteredRecords].sort((a, b) => new Date(a.dateIssued || a.createdAt) - new Date(b.dateIssued || b.createdAt));
        const tableRows = [];

        pdfSortedRecords.forEach(record => {
            const baseDate = new Date(record.dateIssued || record.createdAt).toISOString().split('T')[0];
            ...
            record.items.forEach((item, index) => {
                ...
                tableRows.push([
                    ...
                    `${Number(item.receivedQtyKg || 0).toFixed(2)} kg`,
        ...
        tableRows.push([
            { content: "GRAND TOTAL ISSUED", colSpan: 3, styles: { fontStyle: 'bold', halign: 'right' } },
            { content: `${grandTotalIssuedQty.toFixed(2)} kg`, styles: { fontStyle: 'bold', textColor: [67, 56, 202] } }, // Indigo Color
            "-", "-", "-"
        ]);
