...
const defaultTeaData = [
  ...
  {
    id: 5,
    type: "Christmass tea (reusable bag)",
    amount: 0.005,
    packs: "",
    price: 10,
  },
...
export default function SellingDetailsTable() {
  ...
  const handleFetchData = async (isSilent = false) => {
    ...
    try {
      // api.get භාවිතය (Query params අගට එකතු වේ)
      const response = await api.get(`/api/selling-details?month=${selectedMonth}`);
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
          if (!isSilent) toast.error('Session expired or access denied.', { id: loadToast });
      } else {
  ...
  const handleSaveToDB = async () => {
    ...
    try {
      ...
      // api.post භාවිතය
      await api.post('/api/selling-details', payload);
  ...
  const totalUsd = tableData.reduce(
    (sum, row) => sum + (Number(row.packs) || 0) * (Number(row.price) || 0),
    0,
  );
  const totalLkr = totalUsd * exchangeRate;

  const getPdfData = () => {
    const recordsToPrint = tableData.filter(
      (row) => row.packs !== "" && Number(row.packs) > 0,
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-zinc-950 transition-colors duration-300">
      <div className="p-3 sm:p-5 md:p-8 max-w-[1400px] mx-auto font-sans relative">
          ...
          <AlertDialogContent className="bg-white dark:bg-zinc-900 rounded-2xl border-gray-100 dark:border-zinc-800 shadow-xl max-w-sm sm:max-w-md w-[90vw] transition-colors p-5 sm:p-6">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="text-sm sm:text-base text-gray-500 dark:text-gray-400">
                You have unsaved packs or rate changes. You must save these
                records to the database before generating the PDF report to
        ...
        <div className="sticky top-0 z-40 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-md -mt-3 -mx-3 p-3 sm:-mt-5 sm:-mx-5 sm:pt-6 sm:pb-4 sm:px-5 md:-mt-8 md:-mx-8 md:pt-8 md:pb-4 md:px-8 mb-5 sm:mb-8 border-b border-gray-100 dark:border-zinc-800 shadow-sm flex flex-col min-[850px]:flex-row justify-between items-start min-[850px]:items-center gap-4 transition-colors duration-300">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold text-[#1B6A31] dark:text-[#8CC63F] flex items-center gap-2">
              <DollarSign className="w-6 h-6 sm:w-7 sm:h-7 md:w-[28px] md:h-[28px]" />{" "}
              Monthly Selling Details
            </h2>
            <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1 font-medium">
              Manage and export monthly sales data
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 w-full min-[850px]:w-auto justify-stretch sm:justify-end items-stretch sm:items-center">
            ...
            >
              <RefreshCw
                className={`w-4 h-4 sm:w-[16px] sm:h-[16px] ${isFetching ? "animate-spin" : ""}`}
              />{" "}
              Sync Data
            </button>
        ...
        {isViewer && (
          <div className="mb-5 sm:mb-6 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800/50 text-blue-700 dark:text-blue-300 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg flex items-start sm:items-center gap-2.5 sm:gap-3 transition-colors">
            ...
            <p className="text-xs sm:text-sm font-medium">
              You are logged in as a <strong>Viewer</strong>. You can view data
              and download reports. Editing and saving are disabled.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-5 sm:mb-8">
          ...
          <div className="bg-gradient-to-br from-orange-50/50 to-white dark:from-orange-900/20 dark:to-zinc-900 p-4 sm:p-6 rounded-xl border border-orange-200 dark:border-orange-900/50 shadow-lg shadow-orange-900/5 h-fit relative overflow-hidden transition-colors">
            ...
            <div className="flex flex-col gap-1 sm:gap-1.5 relative z-10">
              <label className="text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 flex items-center gap-1">
                1 USD = (LKR)
              </label>
              <div className="relative">
                <span className="absolute left-3 top-2.5 sm:top-3 text-gray-400 dark:text-gray-500 text-xs sm:text-sm font-bold">
                  Rs.
                </span>
                <input
                  type="number"
        ...
        {/* Main Table */}
        <div
          className={`bg-white dark:bg-zinc-900 rounded-xl shadow-md overflow-hidden mb-8 sm:mb-12 min-h-[300px] border transition-colors ${isViewer ? "border-gray-200 dark:border-zinc-800 opacity-95" : "border-gray-200 dark:border-zinc-800"}`}
        >
          ...
          <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-zinc-700 w-full p-0 sm:p-4">
            <table className="w-full border-collapse min-w-[700px] lg:min-w-max">
              <thead>
                <tr>
                  <th className="sticky left-0 z-20 px-3 sm:px-4 py-3 sm:py-4 font-extrabold text-[9px] sm:text-xs tracking-wider uppercase border-b-2 border-gray-200 dark:border-zinc-700 border-r text-[#1a1a1a] dark:text-gray-200 bg-[#f9f9f9] dark:bg-zinc-800 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] text-center transition-colors">
                    Type of tea
                  </th>
                  <th className="px-2 sm:px-4 py-3 sm:py-4 font-extrabold text-[9px] sm:text-xs tracking-wider uppercase border-b-2 border-gray-200 dark:border-zinc-700 border-r text-[#2e6b3b] dark:text-green-400 bg-[#f4f9f4] dark:bg-green-900/20 text-center transition-colors">
                    Amount (kg)
                  </th>
                  <th className="px-2 sm:px-4 py-3 sm:py-4 font-extrabold text-[9px] sm:text-xs tracking-wider uppercase border-b-2 border-gray-200 dark:border-zinc-700 border-r text-[#2858b4] dark:text-blue-400 bg-[#f0f5fd] dark:bg-blue-900/20 text-center transition-colors">
                    Number of Packs
                  </th>
                  <th className="px-2 sm:px-4 py-3 sm:py-4 font-extrabold text-[9px] sm:text-xs tracking-wider uppercase border-b-2 border-gray-200 dark:border-zinc-700 border-r text-[#d66b2d] dark:text-orange-400 bg-[#fdf7f2] dark:bg-orange-900/20 text-center transition-colors">
                    Price per one (USD)
                  </th>
                  <th className="px-2 sm:px-4 py-3 sm:py-4 font-extrabold text-[9px] sm:text-xs tracking-wider uppercase border-b-2 border-gray-200 dark:border-zinc-700 border-r text-[#1a1a1a] dark:text-gray-200 bg-[#f9f9f9] dark:bg-zinc-800 text-center transition-colors">
                    Total (USD)
                  </th>
                  <th className="px-2 sm:px-4 py-3 sm:py-4 font-extrabold text-[9px] sm:text-xs tracking-wider uppercase border-b-2 border-gray-200 dark:border-zinc-700 border-r text-[#b81d1d] dark:text-red-300 bg-[#fcedec] dark:bg-red-900/20 text-center transition-colors">
                    Total (LKR)
                  </th>
                </tr>
              </thead>
              <tbody>
                {tableData.map((row) => {
                  const calculatedUsd =
                    (Number(row.packs) || 0) * (Number(row.price) || 0);
                  const calculatedLkr = calculatedUsd * exchangeRate;
                    ...
                    >
                      ...
                      <td className="p-1 sm:p-3 text-xs sm:text-sm font-bold border-b border-r border-gray-200 dark:border-zinc-700 text-center transition-colors">
                        <input
                          ...
                          value={row.packs}
                          onChange={(e) =>
                            handleInputChange(row.id, "packs", e.target.value)
                          }
                          disabled={isViewer}
                      ...
                      <td className="p-2 sm:p-3 text-xs sm:text-sm font-bold border-b border-r border-gray-200 dark:border-zinc-700 text-center text-[#1a1a1a] dark:text-gray-200 bg-gray-50/50 dark:bg-zinc-800/50 transition-colors">
                        {calculatedUsd > 0 ? calculatedUsd.toFixed(2) : "0"}
                      </td>
                      <td className="p-2 sm:p-3 text-xs sm:text-sm font-black border-b border-r border-gray-200 dark:border-zinc-700 text-center text-[#b81d1d] dark:text-red-200 bg-red-50/30 dark:bg-red-900/10 transition-colors">
                        {calculatedLkr > 0
                          ? calculatedLkr.toLocaleString()
                          : "0"}
                      </td>
                ...
                <tr className="bg-[#fcedec] dark:bg-zinc-800 border-t-2 border-gray-300 dark:border-zinc-600 transition-colors">
                  <td
                    colSpan="4"
                    className="sticky left-0 z-10 p-2 sm:p-3 text-[10px] sm:text-sm font-bold text-[#1a1a1a] dark:text-gray-200 text-right pr-3 sm:pr-5 border-r border-gray-200 dark:border-zinc-700 bg-[#fcedec] dark:bg-zinc-800 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] whitespace-nowrap"
                  >
                    GRAND TOTAL
                  </td>
                  <td className="p-2 sm:p-3 text-xs sm:text-md font-bold text-[#1a1a1a] dark:text-red-400 text-center border-r border-gray-200 dark:border-zinc-700">
                    $.{totalUsd.toFixed(2)}
                  </td>
                  <td className="p-2 sm:p-3 text-xs sm:text-md font-black text-[#b81d1d] dark:text-red-400 text-center border-r border-gray-200 dark:border-zinc-700">
