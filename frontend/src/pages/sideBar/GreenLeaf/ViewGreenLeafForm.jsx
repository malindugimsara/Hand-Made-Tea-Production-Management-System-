    ...
    const fetchRecords = async () => {
        ...
        try {
            ...
            const processedData = dehydratorData.map(rec => {
                ...
                // Try to find matching Labour record
                const labMatch = labourData.find(l => new Date(l.date).toISOString().split('T')[0] === recDate);

                return {
                    ...rec,
                    isEdited,
                    lastUpdatedDate,
                    selectionWorkers: labMatch ? labMatch.workerCount : '-',
                    rollingType: labMatch ? labMatch.rollingType : '-',
                    rollingWorkers: (labMatch && labMatch.rollingType === 'Hand Rolling') ? labMatch.rollingWorkerCount : '-'
                };
            });
    ...
    const getPdfData = () => {
        const tableRows = filteredRecords.map(record => {
            ...
            return [
                pdfDateCell,
                record.trial,
                record.selectionWorkers,
                `${record.rollingType}\n${record.rollingWorkers !== '-' ? '('+record.rollingWorkers+' wkrs)' : ''}`,
                record.meterStart,
                record.meterEnd,
                record.totalUnits,
            ...
            <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-[#1B6A31] dark:text-green-500 flex items-center gap-2"> 
                        <Leaf size={28} /> Dehydrator Machine Records
                    </h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Overview of Dehydrator Electricity, Time Usage & Labour</p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <PDFDownloader 
                        ...
                        headers={["Date", "Trial", "Sel. Lab.", "Rolling", "Start Meter", "End Meter", "Total Pts", "Time (Hrs)"]}
                        data={getPdfData()}
                        ...
                        <table className="w-full text-sm text-left border-collapse whitespace-nowrap">
                            <thead>
                                <tr className="bg-gray-50 dark:bg-zinc-950/50 text-gray-500 dark:text-gray-400 uppercase text-xs tracking-wider border-b border-gray-200 dark:border-zinc-800 transition-colors">
                                    ...
                                    <th rowSpan="2" className="px-4 py-3 font-bold text-blue-700 dark:text-blue-400 border-r border-gray-200 dark:border-zinc-800 bg-blue-50/50 dark:bg-blue-950/20 align-bottom text-center w-32">
                                ...
                                <tr className="bg-gray-50 dark:bg-zinc-950/50 text-gray-500 dark:text-gray-400 text-xs border-b border-gray-200 dark:border-zinc-800 transition-colors">
                                    {/* Labour Sub-headers */}
                                    <th className="px-3 py-2 font-medium bg-blue-50/50 dark:bg-blue-950/20 text-center border-r border-gray-200/60 dark:border-zinc-800 w-24">Selection</th>
                                    <th className="px-3 py-2 font-medium bg-blue-50/50 dark:bg-blue-950/20 text-center border-r border-gray-200 dark:border-zinc-800 w-32">Rolling</th>
                                ...
                                {filteredRecords.length > 0 ? (
                                    filteredRecords.map((record) => (
                                        <tr key={record._id} className="hover:bg-gray-50/80 dark:hover:bg-zinc-800/50 transition-colors group">
                                            ...
                                            {/* Labour Values */}
                                            <td className="px-3 py-3 text-center border-r border-gray-100 dark:border-zinc-800 bg-blue-50/10 dark:bg-blue-900/10 align-top">
                                                <div className="mt-1 font-bold text-gray-600 dark:text-gray-300">{record.selectionWorkers}</div>
                                            </td>
                                            <td className="px-3 py-3 text-center border-r border-gray-100 dark:border-zinc-800 bg-blue-50/10 dark:bg-blue-900/10 align-top">
                                                <div className="flex flex-col items-center gap-0.5 mt-1">
                                                    <span className="text-gray-700 dark:text-gray-300 font-medium">{record.rollingType}</span>
                                                    {record.rollingWorkers !== '-' && (
                                                        <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 rounded border border-blue-100 dark:border-blue-800/50 mt-1">
                                                            {record.rollingWorkers} wkrs
                                                        </span>
                                                    )}
                                                </div>
                                            </td>
                                    ...
                                    <tr>
                                        <td colSpan={isViewer ? "8" : "9"} className="p-16 text-center text-gray-400 dark:text-zinc-600">
