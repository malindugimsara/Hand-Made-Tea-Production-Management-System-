...
export default function WeightAverage() {
  ...
  routeOptions.forEach((r) => {
    ...
    daysArray.forEach((day) => {
      // Setup base Total Kg and Product Kg fields
      matrixData[routeKey][day] = { totalLeafQty: 0, bestProd: 0, belowBestProd: 0, poorProd: 0 };
    });
  });

  // 2. Populate matrix with database records
  records.forEach((r) => {
    // CRITICAL FIX: Ignore LeafCollector samples entirely
    ...
    if (matrixData[recordRouteKey] && matrixData[recordRouteKey][day]) {
      // Get the Total Leaf Qty in Kg (Base value)
      const dailyTotalKg = Number(r.totalLeafQty) || 0;

      // Calculate actual Product (Kg) using the sample percentages
      const bestProd = dailyTotalKg * ((Number(r.bestPercentage) || 0) / 100);
      const belowBestProd = dailyTotalKg * ((Number(r.belowBestPercentage) || 0) / 100);
      const poorProd = dailyTotalKg * ((Number(r.poorPercentage) || 0) / 100);
  ...
  const generateSimplifiedPDF = async () => {
    ...
    try {
      ...
      const drawPdfTable = (title, dataKey, themeColorRgb) => {
          ...
          Object.keys(matrixData).forEach((routeKey) => {
              ...
              body.push([
                  ...
                  rowSumTotalQty > 0 ? `${rowSumProduct.toFixed(2)} Kg` : "-",
      ...
      drawPdfTable("1. BEST TEA MATRIX SUMMARY", "bestProd", [27, 106, 49]); // Green
      drawPdfTable("2. BELOW BEST TEA MATRIX SUMMARY", "belowBestProd", [217, 119, 6]); // Orange
      drawPdfTable("3. POOR TEA MATRIX SUMMARY", "poorProd", [220, 38, 38]); // Red
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, createSheetData("bestProd", `BEST TEA MATRIX - ${selectedMonth}`, "1B6A31"), "Best Tea");
      XLSX.utils.book_append_sheet(wb, createSheetData("belowBestProd", `BELOW BEST TEA - ${selectedMonth}`, "D97706"), "Below Best");
      XLSX.utils.book_append_sheet(wb, createSheetData("poorProd", `POOR TEA MATRIX - ${selectedMonth}`, "DC2626"), "Poor Tea");
    ...
    <div className="p-4 sm:p-8 max-w-[95vw] mx-auto font-sans relative min-h-screen transition-colors duration-300">
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          {/* 1. BEST TEA TABLE */}
          {renderTable("1. BEST TEA MATRIX (Kg)", "bestProd", {
          ...
          {renderTable("2. BELOW BEST TEA MATRIX (Kg)", "belowBestProd", {
          ...
          {/* 3. POOR TEA TABLE */}
          {renderTable("3. POOR TEA MATRIX (Kg)", "poorProd", {
