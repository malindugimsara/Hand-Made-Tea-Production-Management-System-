...
export default function LoftLeafCount() {
  ...
  const [isFactoryRouteDropdownOpen, setIsFactoryRouteDropdownOpen] =
    useState(false);
  const [isCollectorRouteDropdownOpen, setIsCollectorRouteDropdownOpen] =
    useState(false);
  ...
  useEffect(() => {
    ...
    function handleClickOutside(event) {
      if (
        factoryRouteDropdownRef.current &&
        !factoryRouteDropdownRef.current.contains(event.target)
      ) {
        setIsFactoryRouteDropdownOpen(false);
      }
      if (
        collectorRouteDropdownRef.current &&
        !collectorRouteDropdownRef.current.contains(event.target)
      ) {
  ...
  const factoryStats = calculateStats(
    factoryForm.bestQty,
    factoryForm.belowBestQty,
  );
  const collectorStats = calculateStats(
    collectorForm.bestQty,
    collectorForm.belowBestQty,
  );
    ...
    if (formType === "factory") {
      setFactoryForm((prev) => {
        ...
        if (name === "bestQty" || name === "belowBestQty") {
          ...
          const bb =
            name === "belowBestQty" ? val : Number(prev.belowBestQty) || 0;
  ...
  const handleAddToList = (e, formType) => {
    ...
    if (
      isFactory &&
      (!currentForm.officerName.trim() || !currentForm.totalLeafQty)
    ) {
      toast.error(
        "Please fill all Factory specific fields (Officer, Total Leaf Qty)!",
      );
      return;
    }
    ...
    toast.success(
      `Added ${isFactory ? "Factory" : "Collector"} Sample to list!`,
    );
  };
  ...
  const handleEditChange = (e) => {
    ...
    setEditFormData((prev) => {
      ...
      if (name === "bestQty" || name === "belowBestQty") {
        const b =
          name === "bestQty" ? Number(value) || 0 : Number(prev.bestQty) || 0;
        const bb =
          name === "belowBestQty"
            ? Number(value) || 0
            : Number(prev.belowBestQty) || 0;
  ...
  const handleSaveEdit = () => {
    setPendingRecords(
      pendingRecords.map((r) => (r.id === editingId ? editFormData : r)),
    );
    setEditingId(null);
    setEditFormData(null);
            ...
            <tbody>
              {filteredRecords.map((item) => {
                ...
                return (
                  <tr
                    key={item.id}
                    className="bg-white border-b dark:bg-zinc-900 dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-800/50"
                  >
                        ...
                        <td className="px-4 py-3 text-center">
                          {isEditing ? (
                            <input
                              type="number"
                              name="totalLeafQty"
                              value={data.totalLeafQty || ""}
                              onChange={handleEditChange}
                              className="w-20 p-1 border border-gray-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 rounded text-center outline-none focus:ring-1 focus:ring-teal-500"
                            />
                          ) : (
                            <span className="font-bold text-teal-600 dark:text-teal-400">
                              {data.totalLeafQty
                                ? `${data.totalLeafQty} Kg`
                                : "-"}
                            </span>
                          )}
                        </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      {isEditing ? (
                        <input
                          ...
                          value={data.bestQty}
                          onChange={handleEditChange}
                          className="w-16 p-1 border border-gray-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 rounded text-center outline-none focus:ring-1 focus:ring-teal-500"
                        />
                      ) : (
                      ...
                      {isEditing ? (
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            className="text-green-600 hover:text-green-800 font-bold text-xs bg-green-100 px-2 py-1 rounded"
                          >
                            Save
                          </button>
                          <button
                            onClick={handleCancelEdit}
                            className="text-gray-600 hover:text-gray-800 font-bold text-xs bg-gray-200 px-2 py-1 rounded"
                          >
                            Cancel
                          </button>
                        </div>
    ...
    <div className="p-4 sm:p-8 max-w-[900px] mx-auto font-sans min-h-screen transition-colors duration-300 relative">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          >
            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-200 mb-6 flex items-center gap-2 border-b border-gray-100 dark:border-zinc-800 pb-3">
              <Factory size={20} className="text-gray-800 dark:text-gray-400" />{" "}
              Factory Sample Entry
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
              <div className="p-4 bg-green-50/50 rounded-xl border border-green-100">
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange(e, "factory")}
                  required
                  className="w-full p-2.5 mb-3 border border-green-200 rounded-lg focus:ring-2 focus:ring-[#8CC63F] outline-none"
                />
              ...
              <div className="p-4 bg-yellow-50/50 rounded-xl border border-yellow-100">
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange(e, "factory")}
                  required
                  className="w-full p-2.5 mb-3 border border-yellow-200 rounded-lg focus:ring-2 focus:ring-yellow-400 outline-none"
                />
              ...
              <div className="p-4 bg-red-50/50 rounded-xl border border-red-100">
                <label className="block text-xs font-bold text-red-700 mb-2 uppercase">
                  Poor Leaf (g)
                </label>
                <input
                  type="number"
                  ...
                  disabled
                  className="w-full p-2.5 mb-3 border border-red-200 rounded-lg bg-gray-100 font-bold text-red-700 cursor-not-allowed"
                />
            ...
            <button
              type="submit"
              className="mt-6 w-full py-3 rounded-xl bg-gray-800 text-white font-bold hover:bg-gray-900 transition-all shadow-md flex items-center justify-center gap-2"
            >
        ...
        <div>
          ...
          >
            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-200 mb-6 flex items-center gap-2 border-b border-gray-100 dark:border-zinc-800 pb-3">
              <Users size={20} className="text-teal-600" /> Leaf Collector's
              Sample Entry
            </h3>
            ...
            <button
              type="submit"
              className="mt-6 w-full py-3 rounded-xl bg-teal-600 text-white font-bold hover:bg-teal-700 transition-all shadow-md flex items-center justify-center gap-2"
            >
